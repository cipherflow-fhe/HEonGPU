...
namespace
{
    ...
    cudaStream_t table_build_stream(cudaStream_t stream)
    {
        ...
        std::size_t n2)
    {
	    ...
	    void nwt_2d_radix8_forward_inplace_batched(
	        std::uint64_t* inout, const DNTTTable& ntt_tables,
	        std::size_t coeff_modulus_size, std::size_t poly_count,
        std::size_t start_modulus_idx, const cudaStream_t& stream)
    {
        if (start_modulus_idx != 0)
        {
            throw std::invalid_argument(
                "batched PhantomNTT requires compact HEonGPU rows with start_modulus_idx == 0");
        }
        ...
        constexpr std::size_t per_block_memory = blockDimNTT.x * per_thread_sample_size * sizeof(std::uint64_t);

        batched_inplace_fnwt_radix8_phase1<<<
            gridDimNTT, (phase1_sample_size / 8) * per_block_pad,
            (phase1_sample_size + per_block_pad + 1) * per_block_pad * sizeof(std::uint64_t), stream>>>(
            inout, 
            ntt_tables.twiddle(), 
	    ...
	    void nwt_2d_radix8_backward_inplace_batched(
	        std::uint64_t* inout, const DNTTTable& ntt_tables,
	        std::size_t coeff_modulus_size, std::size_t poly_count,
	        std::size_t start_modulus_idx, const cudaStream_t& stream)
	    {
	        ...
	        {
	            throw std::invalid_argument(
	                "batched PhantomINTT requires compact HEonGPU rows with start_modulus_idx == 0");
	        }
	        ...
	        batched_inplace_inwt_radix8_phase2<<<
	            ...
	            ntt_tables.itwiddle(), 
	            ntt_tables.itwiddle_shoup(),
	            ntt_tables.n_inv_mod_q(), 
	            ntt_tables.n_inv_mod_q_shoup(),
	            ntt_tables.modulus(), 
	            coeff_modulus_size, 
	            start_modulus_idx,
	            poly_count, 
    ...
    std::shared_ptr<PhantomNttTables> make_phantom_ntt_tables_from_heongpu_roots(
        ...
        cudaStream_t stream)
    {
        stream = table_build_stream(stream);

        auto out = std::make_shared<PhantomNttTables>();
        out->n_power = n_power;
        out->n = std::size_t{1} << n_power;
        out->mod_count = static_cast<int>(moduli.size());
        out->table.init(out->n, moduli.size(), stream);
        ...
        for (std::size_t i = 0; i < moduli.size(); ++i){
            ...
            const std::size_t offset = i * out->n;
            std::vector<std::uint64_t> fwd(
                forward_roots.begin() + offset,
                forward_roots.begin() + offset + out->n);
            std::vector<std::uint64_t> inv(
                inverse_roots.begin() + offset,
                inverse_roots.begin() + offset + out->n);
    ...
    bool can_use_phantom_ntt(
        ...
        int mod_count)
    {
        // Route only compact per-polynomial negacyclic RNS rows that match the
        // generated PhantomNTT tables; unsupported GPUNTT shapes fall back.
        if (!tables) return false;
        if (cfg.n_power != tables->n_power) return false;
        if (cfg.ntt_layout != gpuntt::PerPolynomial) return false;
        if (cfg.reduction_poly != gpuntt::ReductionPolynomial::X_N_plus) return false;
        if (cfg.zero_padding) return false;
        if (mod_count <= 0 || batch_size <= 0) return false;
        if (batch_size % mod_count != 0) return false;
        if (mod_count > tables->mod_count) return false;
        return true;
    }

    bool use_batched_phantom_ntt(const PhantomNttTables& tables)
    {
        return tables.n <= (std::size_t{1} << 16);
    }
        ...
        const std::shared_ptr<PhantomNttTables>& tables)
    {
        ...
        if (use_batched_phantom_ntt(*tables))
        {
            nwt_2d_radix8_forward_inplace_batched(
                data, tables->table, mod_count, poly_count, 0, cfg.stream);
            return;
        }
        ...
        const std::shared_ptr<PhantomNttTables>& tables)
    {
        ...
        if (use_batched_phantom_ntt(*tables))
        {
            nwt_2d_radix8_backward_inplace_batched(
                data, tables->table, mod_count, poly_count, 0, cfg.stream);
            return;
        }
